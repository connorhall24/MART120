...
function createObstacle(index) {
    ...
    obstacle.size = obstacleSize[index % obstacleSize.length];
    obstacle.color = obstacleColors[index % obstacleColors.length];
    obstacle.speed = obstacleSpeeds[index % obstacleSpeeds.length];
...
function moveObstacle(index) {
    ...
    if (
        obstacles[index].x > width + obstacleSize[index % obstacles[index].size] ||
        obstacles[index].x < 0 - obstacleSize[index % obstacles[index].size]
    ) {
...
function mouseClicked() {
    ...
    obstacles[obstacles.length - 1].x = newObstacleX;
    obstacles[obstacles.length - 1].y = newObstacleY;
}
