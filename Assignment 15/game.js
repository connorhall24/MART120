...
function createObstacle(i) {
    ...
    var obstacle = new Obstacle(
        ...
        obstacleSpeeds[Math.floor(Math.random() * obstacleSpeeds.length)]
    );
...
function moveObstacle(index) {
    ...
    if (obstacles[index].x > width) {
        obstacles[index].x = -obstacles[index].width;
        obstacles[index].y = random(height);
    } else if (obstacles[index].x < -obstacles[index].width) {
        obstacles[index].x = width;
        obstacles[index].y = random(height);
    }
    if (obstacles[index].y > height) {
        obstacles[index].y = -obstacles[index].height;
        obstacles[index].x = random(width);
    } else if (obstacles[index].y < -obstacles[index].height) {
        obstacles[index].y = height;
        obstacles[index].x = random(width);
    }
}
